#!/usr/bin/env python3
"""
Write-up figure: Flat Swap Conditions (MotionSwap & Dots50Swap).
Sessions: 260324_1010, 260325_1039.
Saved to: Agents/WriteUps/swap_writeup.png
"""
...
OUT_PATH = os.path.expanduser(
    "~/Projects/ObjectBasedAttention/VRDots/Agents/WriteUps/swap_writeup.png")
...
# ── Title ─────────────────────────────────────────────────────────────────────
fig.text(0.50, y, 'VRDots \u2014 Flat Swap Conditions: Motion Swap & Dots50 Swap',
...
y -= len(intro_lines)*line_h(8.5) + 0.006

stim_lines = textwrap.wrap(STIM_NOTE, width=WRAP_W)
fig.text(LMAR, y, '\n'.join(stim_lines),
    ha='left', va='top', fontsize=8.0, color='#555555',
    linespacing=1.5, style='italic')
y -= len(stim_lines)*line_h(8.0) + 0.014
...
TRAJ_H = 0.345    # total vertical fraction for 6-row trajectory grid
traj_top = y
traj_bot = traj_top - TRAJ_H

TRAJ_COL_W = 0.38   # width of the single trajectory column, centred
traj_left  = 0.50 - TRAJ_COL_W/2
traj_right = 0.50 + TRAJ_COL_W/2
...
for sec_idx, (swap_type, swap_label, swap_color) in enumerate(SWAP_SECTIONS):
    for row_off, is_cued in [(0, True), (1, False)]:
        ...
        ax = fig.add_subplot(traj_gs[grid_row, 0])
    ...
    pos_top = traj_gs[sec_idx*2,   0].get_position(fig).y1
    pos_bot = traj_gs[sec_idx*2+1, 0].get_position(fig).y0
    mid_y   = (pos_top + pos_bot) / 2
    fig.text(traj_right + 0.015, mid_y, swap_label,
    ...
    if sec_idx < 2:
        ...
        fig.add_artist(plt.Line2D([traj_left, traj_right], [sep_y, sep_y],
            transform=fig.transFigure, color='#BBBBBB', lw=1.0, ls='--'))
...
y -= len(an_lines)*line_h(8.0) + 0.014
