#!/usr/bin/env python3
"""
Write-up figure: Depth Baseline (no swap).
Sessions: 260325_1831 & 260325_1914 (0.10 m), 260325_2013 (0.03 m).
Saved to: Agents/WriteUps/depth_baseline_writeup.png
...
OUT_PATH = os.path.expanduser(
    "~/Projects/ObjectBasedAttention/VRDots/Agents/WriteUps/depth_baseline_writeup.png")
...
# ── Title ─────────────────────────────────────────────────────────────────────
fig.text(0.50,y,'VRDots \u2014 Depth Baseline (No Swap)',
    ha='center',va='top',fontsize=15,fontweight='bold',color='#111111')
y-=line_h(15,1.3)+0.006
...
y-=line_h(11,1.3)+0.006

# ── Trajectory grid: 2 cols (Near trans | Far trans) × (CUED motion, CUED depth, UNCUED motion, UNCUED depth) ──
# 4 rows per col: motion-CUED, depth-CUED, motion-UNCUED, depth-UNCUED
# height_ratios: [3,1,3,1]

TRAJ_H=0.270
traj_top=y; traj_bot=traj_top-TRAJ_H
traj_left=0.10; traj_right=0.72
...
# cw_depth per column (translating=CW field depth)
CW_DEPTHS=['N','F']

for col,(ctitle,cw_dep) in enumerate(zip(COL_TITLES,CW_DEPTHS)):
    # CUED motion (row 0)
    ax_cm=fig.add_subplot(traj_gs[0,col])
    ...
    # CUED depth (row 1)
    ax_cd=fig.add_subplot(traj_gs[1,col])
    # In CUED: delayed (B) = CW field; onset at T_B
    # CW field (green, dashed in depth track) is at cw_dep
    draw_depth_panel(ax_cd,cw_depth=cw_dep)
    if col==0: ax_cd.set_ylabel('Depth',fontsize=7,labelpad=4,color='#555555')

    # UNCUED motion (row 2)
    ax_um=fig.add_subplot(traj_gs[2,col])
    draw_motion_panel(ax_um,is_cued=False)
    if col==0: ax_um.set_ylabel('UNCUED',fontsize=9,fontweight='bold',labelpad=4)

    # UNCUED depth (row 3)
    ax_ud=fig.add_subplot(traj_gs[3,col])
    # In UNCUED: non-delayed (A) = CW field; translating at cw_dep
    # delayed (B) = CCW field; at opposite depth
...
],loc='center',ncol=6,fontsize=8.5,frameon=True,framealpha=0.9,edgecolor='#CCCCCC')
y-=TLEG_H+0.012
...
for si,(sid,slabel,scol) in enumerate(SESSIONS):
    rows=load(sid); cnt=analyze(rows)
    ...
    bcolors=['#4488CC','#4488CC','#CC6644','#CC6644']
    hatch=['','///','','///']
    ...
    ax.text((xs[0]+xs[1])/2,-0.09,'Near\nTrans.',ha='center',fontsize=8.5,
            fontweight='bold',color='#4488CC',transform=ax.get_xaxis_transform())
    ax.text((xs[2]+xs[3])/2,-0.09,'Far\nTrans.',ha='center',fontsize=8.5,
            fontweight='bold',color='#CC6644',transform=ax.get_xaxis_transform())
...
ax_bleg.legend(handles=[
    mpatches.Patch(facecolor='#4488CC',alpha=0.82,edgecolor='#333333',label='Near translation (blue)'),
    mpatches.Patch(facecolor='#CC6644',alpha=0.82,edgecolor='#333333',label='Far translation (orange)'),
