#!/usr/bin/env python3
"""
Baseline experiment write-up — multi-page PDF.
US Letter portrait (8.5 × 11 in, 150 DPI), 2 pages.

  Page 1 — Introduction + Stimulus Conditions (trajectory panels)
  Page 2 — Results + Findings + Software notes

Output: Agents/WriteUps/baseline_writeup.pdf
"""
...
OUT_PDF = os.path.expanduser(
    "~/Projects/ObjectBasedAttention/VRDots/Agents/WriteUps/baseline_writeup.pdf")
...
LMAR, RMAR = 0.09, 0.91
TXT_W = RMAR - LMAR
WRAP_W = 115        # characters per line
...
STIM_NOTE = (
    "Each panel shows Field A (solid line) and Field B (dashed line) motion "
    "trajectories over trial time. The grey shaded region marks the 80 ms "
    "translation window. The vertical dashed line marks Field B onset. "
    "Row labels (CUED / UNCUED) indicate which field translates. "
    "Column labels indicate the translating field color and rotation direction. "
    "Line color indicates dot field color (red or green)."
)
...
session_data = [(sid, lbl, analyze(load_session(sid))) for sid, lbl in SESSIONS]
...
# Title
fig1.text(0.50, y, 'VRDots — Baseline Experiment: No Depth, No Swap',
          ha='center', va='top', fontsize=13, fontweight='bold', color='#111111')
y -= lh(13, 1.3) + 0.006

# Sessions line
fig1.text(0.50, y,
    'Sessions: 260323_1534 (pre-v0.2.0)   260324_0716 (v0.2.0)',
...
y -= len(intro_lines) * lh(8.5) + 0.016
...
# Trajectory panels
TRAJ_H = 0.390   # height for the 2-row trajectory grid
traj_top = y
...
for row, col, is_cued, rot0, ca, cb in PANELS:
    ax = fig1.add_subplot(traj_gs[row, col])
...
# Small header
fig2.text(0.50, y, 'VRDots — Baseline Experiment (continued)',
...
for si, (sess_id, sess_label, cnt) in enumerate(session_data):
...
for para in [FINDINGS_1, FINDINGS_2]:
    ...
    fig2.text(LMAR, y, '\n'.join(lines),
        ha='left', va='top', fontsize=8.2, color='#222222', linespacing=1.4)
    y -= len(lines) * lh(8.2, 1.4) + 0.008

y -= 0.008   # section gap

# Software version heading
fig2.text(LMAR, y, 'Software version and known artifacts',
