"""
decoupled_C_traces.py
---------------------
Dot-motion trace figure for Exp_DecoupledDots_005m, C (color swap) condition.
All 16 permutations as 8 CUED/UNCUED pairs. Heading = 0°.

C swap: at T_START, field colors exchange.
Motion (rotation/translation) unchanged from N condition.
Depth planes unchanged.

Visual encoding:
  Color: changes at T_START (pre: original field colors; post: swapped)
  Fill:  Filled = Near depth plane  |  Open = Far depth plane  (constant)
  Alpha: early frames faint → later frames opaque

Output: Agents/Figures/decoupled_C_traces.pdf / .png
"""
...
def draw_panel(ax, rot_cfg, b_green, b_near, cued, row_label='', col_title=''):
    ...
    # Post-swap colors exchange between fields
    col_A2 = col_B
    col_B2 = col_A
    b_cw   = (rot_cfg == 1)
    a_cw   = (rot_cfg == 0)
    a_near = not b_near   # depth unchanged throughout
    ...
    # (subfield, pre-swap color, post-swap color, depth near — constant)
    layers = [(s0, col_A, col_A2, a_near),
              (s1, col_A, col_A2, a_near),
              (s2, col_B, col_B2, b_near),
              (s3, col_B, col_B2, b_near)]

    for sf, col_pre, col_post, is_near in layers:
