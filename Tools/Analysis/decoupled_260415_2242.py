#!/usr/bin/env python3
"""
decoupled_260415_2242.py

Single-session figure for DecoupledDots_005m_v2, session 260415_2242 (n=512).

⚠️  DISPARITY SIGN BUG: DelayedFieldDepth labels are INVERTED in this session.
    'N' in file → observer experienced FAR (uncrossed) disparity
    'F' in file → observer experienced NEAR (crossed) disparity
    Near/Far panel uses corrected (perceptual) labels.
    Overall cueing and swap-type analyses are unaffected.

viewDistance=2m (pre-fix; disparity ~2.7 arcmin vs 4.8 arcmin in 0847 session).
"""
...
SESSION = '/tmp/quest_pull_latest/vr_dots_session_260415_2242.tsv'

BASE = os.path.normpath(os.path.join(os.path.dirname(__file__),
                                      '../../Agents/SwapPilot/Figures'))
os.makedirs(BASE, exist_ok=True)
OUT = os.path.join(BASE, 'decoupled_260415_2242.png')
...
# Near/Far label inversion: file 'N' = perceptual Far, file 'F' = perceptual Near
DEPTH_CORRECT = {'N': 'Far', 'F': 'Near'}   # file code → perceptual label
...
with open(SESSION, newline='') as f:
    for r in csv.DictReader(f, delimiter='\t'):
        ...
        rows.append(dict(
            cond    = r['Cond'],
            swap    = r['SwapType'],
            depth_file = r.get('DelayedFieldDepth','?'),     # raw file label (inverted)
            depth_perc = DEPTH_CORRECT.get(r.get('DelayedFieldDepth','?'), '?'),  # corrected
...
fig.suptitle(
    'DecoupledDots_005m_v2  ·  Session 260415_2242  ·  n=512\n'
    '⚠️  Disparity sign bug: Near/Far labels corrected (file N→Far, F→Near perceptually)  '
    '·  viewDist=2m (pre-fix, ~2.7 arcmin disparity)',
...
# ── Row 2, Panel C: Near / Far (corrected labels) ─────────────────────────────

# Perceptual Near = file 'F', perceptual Far = file 'N'
nf_xs  = np.array([0, 1])
nf_w   = 0.33
nf_gap = 0.06

# ordered as perceptual [Near, Far]
depth_file_order = ['F', 'N']    # file codes for [perceptual Near, perceptual Far]
...
for ci, cond in enumerate(['CUED', 'UNCUED']):
    ...
    for di, dep_file in enumerate(depth_file_order):
        k, n = cell(lambda r, c=cond, d=dep_file: r['cond']==c and r['depth_file']==d)
        p    = pct(k, n)
        lo, hi = wilson_ci(k, n)
...
# brackets
for di, dep_file in enumerate(depth_file_order):
    kc, nc_n = cell(lambda r, d=dep_file: r['cond']=='CUED'   and r['depth_file']==d)
    ku, nu_n = cell(lambda r, d=dep_file: r['cond']=='UNCUED' and r['depth_file']==d)
...
ax_nf.set_title('Near / Far (labels corrected for sign bug)\n⚠️ file N→Far, file F→Near',
...
fig.text(0.5, 0.02,
         ...
         'Row 2 right: Near/Far corrected for disparity sign bug (file N=Far, F=Near perceptually)  ·  '
