"""
decoupled_Z_traces.py
---------------------
Dot-motion trace figure for Exp_DecoupledDots_005m, Z (depth swap) condition.
All 16 permutations as 8 CUED/UNCUED pairs. Heading = 0°.

Z swap: at T_START, depth planes exchange between fields.
Motion (rotation/translation) unchanged from N condition.
Colors unchanged.

Visual encoding:
  Color: unchanged throughout (original field colors)
  Fill:  changes at T_START — Near↔Far swap
         Filled = Near  |  Open = Far
  Alpha: early frames faint → later frames opaque

Output: Agents/Figures/decoupled_Z_traces.pdf / .png
"""
...
def draw_panel(ax, rot_cfg, b_green, b_near, cued, row_label='', col_title=''):
    ...
    a_cw   = (rot_cfg == 0)
    a_near = not b_near
    # Post-swap: depth planes exchange
    a_near2 = b_near
    b_near2 = a_near
    ...
    # (subfield, color — constant, pre-swap near, post-swap near)
    layers = [(s0, col_A, a_near, a_near2),
              (s1, col_A, a_near, a_near2),
              (s2, col_B, b_near, b_near2),
              (s3, col_B, b_near, b_near2)]

    for sf, col, near_pre, near_post in layers:
...
fig_all.savefig(os.path.join(BASE, 'decoupled_Z_traces.png'), dpi=150, bbox_inches='tight')
plt.close(fig_all)
print('Saved PNG')

with PdfPages(os.path.join(BASE, 'decoupled_Z_traces.pdf')) as pdf:
    for i in range(0, len(ROWS), 2):
        fig_p = build_figure(ROWS[i:i+2], figsize=(8.5, 11))
        pdf.savefig(fig_p, bbox_inches='tight')
        plt.close(fig_p)
print(f'Saved PDF: {BASE}/decoupled_Z_traces.pdf')
