#!/usr/bin/env python3
"""
session_260413_2051.py

Standard performance figure for session 260413_2051
(DecoupledDots_005m_v2, n=512).
Post-fix session: Start() removed; rotation set per-trial in BuildFromCondition()
so XR tracking is guaranteed initialized at each trial onset.

Panels:
  A — CUED vs UNCUED × swap (N/C/Z/CZ), Near & Far sub-bars
  B — Factor summary (F1 dot, F2 depth disrupt, F3 color, F4 Far−Near)
  C — Response direction bias on wrong trials (% →90° up, % →270° down)

Output:
  Agents/SwapPilot/Figures/decoupled_dots_260413_2051.png
"""
...
OUT = os.path.join(BASE, 'decoupled_dots_260413_2051.png')
...
with open(DATA, newline='') as f:
    for r in csv.DictReader(f, delimiter='\t'):
        ...
        if r.get('EndKey','') in ('timeout','skip','requeue'):
            continue
        td = float(r['TransDeg']); rd = float(r['RespDeg'])
        rows.append(dict(
            cond  = r['Cond'],
            swap  = r['SwapType'],
...
n_total = len(rows)
print(f"Loaded {n_total} trials from {DATA}")
...
kc_N, nc_N = cell(lambda r: r['cond']=='CUED'   and r['swap']=='N')
ku_N, nu_N = cell(lambda r: r['cond']=='UNCUED' and r['swap']=='N')
kc_Z, nc_Z = cell(lambda r: r['cond']=='CUED'   and r['swap']=='Z')
ku_Z, nu_Z = cell(lambda r: r['cond']=='UNCUED' and r['swap']=='Z')
kc_C, nc_C = cell(lambda r: r['cond']=='CUED'   and r['swap']=='C')
kc_CZ,nc_CZ= cell(lambda r: r['cond']=='CUED'   and r['swap']=='CZ')

F1 = pct(kc_N,nc_N) - pct(ku_N,nu_N)
F2 = pct(kc_N,nc_N) - pct(kc_Z,nc_Z)
F3 = pct(kc_N,nc_N) - pct(kc_C,nc_C)
F1xF2 = (pct(kc_N,nc_N) - pct(ku_N,nu_N)) - (pct(kc_Z,nc_Z) - pct(ku_Z,nu_Z))

kF_N, nF_N = cell(lambda r: r['swap']=='N' and r['depth']=='F')
kNr_N,nNr_N= cell(lambda r: r['swap']=='N' and r['depth']=='N')
Far_Near_N = pct(kF_N,nF_N) - pct(kNr_N,nNr_N)

kF_cued, nF_cued   = cell(lambda r: r['swap']=='N' and r['depth']=='F' and r['cond']=='CUED')
kNr_cued,nNr_cued  = cell(lambda r: r['swap']=='N' and r['depth']=='N' and r['cond']=='CUED')
kF_un,   nF_un     = cell(lambda r: r['swap']=='N' and r['depth']=='F' and r['cond']=='UNCUED')
kNr_un,  nNr_un    = cell(lambda r: r['swap']=='N' and r['depth']=='N' and r['cond']=='UNCUED')

up_Z,  nw_Z  = wrong_dir_pct(lambda r: r['swap']=='Z',  90)
up_N,  nw_N  = wrong_dir_pct(lambda r: r['swap']=='N',  90)

print(f"\n--- Session 260413_2051 ---")
print(f"F1  (dot cueing N):         {F1:+.1f}pp  {stars(chi2_p(kc_N,nc_N,ku_N,nu_N))}")
print(f"F2  (depth disrupt CUED):   {F2:+.1f}pp  {stars(chi2_p(kc_N,nc_N,kc_Z,nc_Z))}")
print(f"F3  (color disrupt CUED):   {F3:+.1f}pp  {stars(chi2_p(kc_N,nc_N,kc_C,nc_C))}")
print(f"F1xF2 (CUED N-Z) - (UNCUED N-Z):  {F1xF2:+.1f}pp")
print(f"Far>Near (N cond, all):     {Far_Near_N:+.1f}pp  {stars(chi2_p(kF_N,nF_N,kNr_N,nNr_N))}")
print(f"Far>Near (CUED):            {pct(kF_cued,nF_cued)-pct(kNr_cued,nNr_cued):+.1f}pp  {stars(chi2_p(kF_cued,nF_cued,kNr_cued,nNr_cued))}")
print(f"Far>Near (UNCUED):          {pct(kF_un,nF_un)-pct(kNr_un,nNr_un):+.1f}pp  {stars(chi2_p(kF_un,nF_un,kNr_un,nNr_un))}")
print(f"UP bias Z wrong:            {up_Z:.1f}% (n_wrong={nw_Z})")
print(f"UP bias N wrong:            {up_N:.1f}% (n_wrong={nw_N})")
...
fig.suptitle(
    ...
    'Post-fix: jerk removed + expansion/contraction removed + per-trial rotation (XR tracking stable)\n'
    'CUED (blue) = cue marks translator  ·  Near/Far = translating field depth',
    fontsize=10, fontweight='bold', y=0.99)

gs = gridspec.GridSpec(2, 3, top=0.91, bottom=0.07,
...
bar_w = 0.22; gap = 0.04; group_gap = 0.45
xticks, xlabels = [], []
...
for gi, swap in enumerate(SWAP_ORDER):
    ...
    y_br = 72
    xL = xs[0]-bar_w/2; xR = xs[3]+bar_w/2
    ax_a.plot([xL,xL,xR,xR],[y_br-1.5,y_br,y_br,y_br-1.5], color='#444', lw=0.8)
    ax_a.text((xL+xR)/2, y_br+0.5, f'Δ{eff:+.1f}pp {stars(pval)}',
              ha='center', va='bottom', fontsize=7.5, color='#222', fontweight='bold')

    group_center = (xs[0]+xs[3])/2
    xticks.append(group_center); xlabels.append(SWAP_LABELS[swap])

ax_a.set_xticks(xticks); ax_a.set_xticklabels(xlabels, fontsize=8.5)
...
for fi, (lbl, (k1,n1), (k2,n2), c1, c2) in enumerate(factors):
    ...
    draw_bar(ax_b, x1, k1, n1, c1, width=0.42)
    draw_bar(ax_b, x2, k2, n2, c2, width=0.42)
    ...
    ax_b.text((x1+x2)/2, y_br+0.5, f'{eff:+.1f}pp\n{stars(pval)}',
...
xs = np.arange(len(conditions)); bar_w2 = 0.32; gap2 = 0.06

for i, (lbl, up, dn, n_wrong) in enumerate(conditions):
    x_up = xs[i]-bar_w2/2-gap2/2; x_dn = xs[i]+bar_w2/2+gap2/2
    up_color = '#c0392b' if up > 2*EXPECTED else '#e8a5a0'
    ax_c.bar(x_up, up, bar_w2, color=up_color, zorder=3,
             label='→90° (up)' if i==0 else None)
    ax_c.text(x_up, up+0.8, f'{up:.0f}%', ha='center', va='bottom', fontsize=7,
              color='#222', fontweight='bold' if up > 2*EXPECTED else 'normal')
    ...
    ax_c.text(x_dn, dn+0.8, f'{dn:.0f}%', ha='center', va='bottom', fontsize=7,
              color='#222', fontweight='bold' if dn > 2*EXPECTED else 'normal')
    ax_c.text(xs[i], -6,  lbl,              ha='center', va='top', fontsize=7.5, color='#333')
    ax_c.text(xs[i], -12, f'n_w={n_wrong}', ha='center', va='top', fontsize=6,   color='#777')

ax_c.axhline(EXPECTED,   color='#888',    lw=1.0, ls='--', zorder=2)
ax_c.axhline(2*EXPECTED, color='#c0392b', lw=0.8, ls=':',  alpha=0.6, zorder=2)
n_c = len(conditions)
ax_c.text(n_c-0.4, EXPECTED+0.5,   'expected\n(14.3%)',   fontsize=6.5, color='#888',    va='bottom')
ax_c.text(n_c-0.4, 2*EXPECTED+0.5, '2× expected\n(28.6%)',fontsize=6.5, color='#c0392b', va='bottom', alpha=0.8)
ax_c.set_xticks(xs); ax_c.set_xticklabels(['']*n_c)
ax_c.set_xlim(-0.7, n_c-0.3); ax_c.set_ylim(-16, 95)
...
ax_c.axvline(3.5, color='#ccc', lw=1.2, zorder=1)
ax_c.text(1.5, 90, 'CUED',   ha='center', va='top', fontsize=9, color='#1a5276', fontweight='bold')
ax_c.text(5.5, 90, 'UNCUED', ha='center', va='top', fontsize=9, color='#7b241c', fontweight='bold')
...
fig.text(0.5, 0.01,
         ...
         '†p<.1  *p<.05  **p<.01  ***p<.001  ·  '
         'Direction bias expected ~14.3%/direction (8-AFC)',
